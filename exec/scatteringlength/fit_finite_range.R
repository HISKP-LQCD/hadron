...
##    if(i == 1) {
##      cat((fn(opt.res$par, L=x[jj,5], m=bs[i,4])-bs[i,jj])/x[jj,2], "\n")
##      cat(fn(opt.res$par, L=x[jj,5], m=bs[i,4]), "\n", bs[i,jj], "\n", x[jj,2], "\n")
##      cat(x[jj,5], "\n")
##      cat(opt.res$par, bs[i,4], "\n")
##      print(opt.res)
##    }
  }
  ...
  plot(x=1/x[,5], y=x[,1], type="n", xlim=c(0,0.06), ylim=c(0,0.02), xlab=c("$a/L$"), ylab=c("$a\\delta E(1/L)$"))
  ...
  plotwitherror(x=1/x[,5], y=x[,1], dy=x[,2], rep=TRUE, col="blue")
