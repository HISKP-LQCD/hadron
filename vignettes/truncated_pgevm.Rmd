...
[arxiv.org/abs/2411.14981](https://arxiv.org/abs/2411.14981) and .... It
...
pion.truncated.pgevm <- bootstrap.truncated.pgevm(pioncf.boot, N=pioncf.boot$Time,
                                      t0=t0, n=24-t0, error.weights=TRUE, symmetric=TRUE)
...
The (close-to) optimal fit result with $k = `r trunc`$ states reads $E =`r pion.truncated.pgevm.efm$t0[trunc]` \pm `r pion.truncated.pgevm.efm$se[trunc]`$.

Once again, we can now plot the different effective masses.
