...
plot(pioncf.boot, ylab="C(t)", xlab="t/a", xlim=c(0,48), log="y")
```
...
pion.lanczos <- fit.effectivemass(pion.lanczos, t1=10, t2=21, useCov=TRUE)
```
...
An alternative way to include late time slices is to use the `block.Delta=TRUE` option in the `bootstrap.pgevm` function. This will use the first and last `n` rows and columns of the Hankel matrix.

```{r, warn=FALSE}
pion.pgevm.block <- bootstrap.pgevm(pioncf.boot, N=pioncf.boot$Time, block.Delta=TRUE)
pion.pgevm.block.efm <- pgevm2effectivemass(pion.pgevm.block, errortype="dbboot",
                                       bias_correction=TRUE)
pion.pgevm.block.efm2 <- pgevm2effectivemass(pion.pgevm.block, errortype="dbboot",
                                       bias_correction=TRUE, average.negE=TRUE)
```

We can now plot the different effective masses. Note that for the blocked version `n` denotes the dimension of a block, i.e. the full dimension is `2n`.

```{r}
plot(pion.pgevm.efm, ylab="Meff", xlab="n", xlim=c(0,24), ylim=c(.12,.155),
...
plot(pion.pgevm.block.efm, rep=TRUE, col="green", pch=24, xshift=0.45)
plot(pion.pgevm.block.efm2, rep=TRUE, col="brown", pch=25, xshift=0.6)
```

For noiseless data and infinite precision the best convergence is achieved by starting around half time `T/2` and expanding the Hankel matrix from there. We can define this order using `custom.indices`:

```{r, warn=FALSE}
n.half <- floor(pioncf.boot$Time/4)
custom.indices <- c(rbind(n.half:1, (n.half+1):(2*n.half)))
print(custom.indices)
pion.pgevm.inout <- bootstrap.pgevm(pioncf.boot, N=pioncf.boot$Time, n.max=2*n.half,
                                       custom.indices=custom.indices)
pion.pgevm.inout.efm <- pgevm2effectivemass(pion.pgevm.inout, errortype="dbboot",
                                       bias_correction=TRUE)
pion.pgevm.inout.efm2 <- pgevm2effectivemass(pion.pgevm.inout, errortype="dbboot",
                                       bias_correction=TRUE, average.negE=TRUE)
```

Even at the mild noise level of the pion this method is very unstable, as was to be expected.

```{r}
plot(pion.pgevm.efm, ylab="Meff", xlab="n", xlim=c(0,24), ylim=c(.12,.155),
                     col="blue", pch=21, xshift=0)
plot(pion.pgevm.efm3, rep=TRUE, col="red", pch=22, xshift=0.15)
plot(pion.pgevm.block.efm2, rep=TRUE, col="darkorange", pch=23, xshift=0.3)
plot(pion.pgevm.inout.efm, rep=TRUE, col="green", pch=24, xshift=0.45)
plot(pion.pgevm.inout.efm2, rep=TRUE, col="brown", pch=25, xshift=0.6)
```
