...
void getboth(double * res, double * r0sqTwoBmu, double * par, const int N, 
  ...
  if(fitnnlo) {
    for(i = 0; i < N; i++) {
      ...
      res[i+N] = r0sqTwoBmu[i]*(1. + xi*rln3 + asq/par[3+fitasq]/par[3+fitasq]*par[npar-2] +
    ...
    for(i = 0; i < N; i++) {
      ...
      res[i+N] = r0sqTwoBmu[i]*(1.0 + asq/par[3+fitasq]/par[3+fitasq]*par[npar-2] + r0sqTwoBmu[i]*(rln3/(4.0*pi*par[2])/(4.0*pi*par[2]) ));
    }
  }
  return;
...
SEXP getbothc(SEXP r0sqTwoBmu_, SEXP par_, SEXP N_, SEXP fitnnlo_, SEXP fitkmf_, SEXP fitasq_) {
  ...
  PROTECT(res_ = NEW_NUMERIC(2*N));
  res = NUMERIC_POINTER(res_);
  getboth(res, r0sqTwoBmu, par, N, npar, fitnnlo, fitkmf, (int)fitasq);  
  UNPROTECT(7);
  return(res_);
}
