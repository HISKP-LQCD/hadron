...
SEXP LuscherZeta(SEXP qsq_, SEXP l_, SEXP m_, SEXP dvec_, SEXP gamma_, SEXP lambda_) {
  ...
  luscherZeta(ires, qsq, l, m, gamma, lambda, dvec);
...
SEXP LuscherZetaArray(SEXP qsq_, SEXP n_, SEXP l_, SEXP m_, SEXP dvec_, SEXP gamma_, SEXP lambda_) {
  ...
  for(int i = 0; i < n; i ++) {
    luscherZeta(ires, qsq[i], l, m, gamma, lambda, dvec);
    resp[i].r = ires[0];
    resp[i].i = ires[1];
  }
