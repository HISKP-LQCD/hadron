...
SEXP LuscherZetaArray(SEXP qsq_,
                      SEXP n_,
                      SEXP l_,
                      SEXP m_,
                      SEXP dvec_,
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (rstatus[0] != 0) {
      warning(
          "GSL error in evaluation of first contribution to Luescher Zeta function, "
          "status code %d for element %d\n",
          rstatus[0],
          i + 1);
      resp[i].r = NA_REAL;
      resp[i].i = NA_REAL;
      ...
      warning(
          "GSL error in evaluation of second contribution to Luescher Zeta function, "
          "status code %d for element %d\n",
          rstatus[1],
          i + 1);
      resp[i].r = NA_REAL;
      ...
      warning(
          "GSL error in evaluation of third contribution to Luescher Zeta function, "
          "status code %d for element %d\n",
          rstatus[2],
          i + 1);
      resp[i].r = NA_REAL;
