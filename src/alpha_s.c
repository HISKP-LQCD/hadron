...
SEXP alphas_c(SEXP mu_, SEXP nl_, SEXP lam0_, SEXP Nc_, SEXP Nf_) {
  ...
  b3 = (150653./486. - 44./9.*Z3)*Nc*Nc*Nc*Nc + 
    ...
    (-2102./27. + 176./9.*Z3)*Cf*Cf*Nc*Nf +
    23.*Cf*Cf*Cf*Nf + (3965./162. + 56./9.*Z3)*Nc*Nc*Nf*Nf + 
    (338./27. - 176./9.*Z3)*Cf*Cf*Nf*Nf +
    (4288./243. + 112./9.*Z3)*Cf*Nc*Nf*Nf + 53./243.*Nc*Nf*Nf*Nf +
    154./243.*Cf*Nf*Nf*Nf + 
    (-10./27. + 88./9.*Z3)*Nc*Nc*(Nc*Nc+36.) +
    (32./27. - 104./9.*Z3)*Nc*(Nc*Nc+6)*Nf +
    (-22./27. + 16./9.*Z3)*(Nc*Nc*Nc*Nc - 6.*Nc*Nc + 18.)/(Nc*Nc)*Nf*Nf;
  ...
  als3 = als2 + als0*als0*als0*als0*(b1*b1*b1*(-LL2*LL2*LL2+5./2.*LL2*LL2+2*LL2-1./2.)-
				     3.*b1*b2*LL2 + b3/2.);
  PROTECT(res = NEW_NUMERIC(1));
  resp = NUMERIC_POINTER(res);

  if(nl == 0) resp[0] = (als0/(4.*pi));
  else if(nl == 1) resp[0] = (als1/(4.*pi));
  else if(nl == 2) resp[0] = (als2/(4.*pi));
  else resp[0] = (als3/(4.*pi));
  UNPROTECT(6);
  return(res);
}
