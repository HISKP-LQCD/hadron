...
/*   double parm[] = {0., 0., 0., 0., 0., 0.}; */
  ...
  for(i = 0; i < n; i++) {
    S4mpi[i] = (13./3.)*gg[1] * mmB0[i] - (1./3.)*(40.*gg[1] + 32.*gg[2] + 26.*gg[3])*mmB2[i];
    S4fpi[i] = (1./6.)*(8*gg[1] - 13.*gg[2])* mmB0[i] - 
      (1./3.)*(40.*gg[1] - 12.*gg[2] - 8.*gg[3] - 13.*gg[4])*mmB2[i];
  }
  for(i = 0; i < n; i++) {
    I2mpi[i] = -mmB0[i];
...
SEXP cdh_c(SEXP rev, SEXP L1, SEXP L2, SEXP L3, SEXP L4, SEXP F0, SEXP a, SEXP L, 
  ...
  fscdh(revp[0], L1p[0], L2p[0], L3p[0], L4p[0], F0p[0], ap[0], Lp, mpip, fpip, N, mpiFV, fpiFV);
