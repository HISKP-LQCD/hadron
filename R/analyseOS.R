analyse.os <- function(N, kappa, t1, t2, boot.R, boot.l, muvalues, ename, starti=0) {
  ...
  for (i in starti:N) {
    for (j in i:N) {
      ...
      res <- smearedpion(cmicor, t1=t1, t2=t2, debug=FALSE, method="all", mu1=mu1, mu2=mu2, kappa=kappa, boot.R=boot.R, boot.l=boot.l)
