...
#' @param dx numeric vector of the same length as \code{x} or missing. Errors
#' of the values.
...
#' @return A matrix with as many columns as there are variables in \code{x} and
#' as many rows as \code{boot.R}.
#' @seealso Other NLS fit functions: \code{\link{bootstrap.nlsfit}()},
#' \code{\link{parametric.bootstrap.cov}()},
#' \code{\link{parametric.nlsfit.cov}()}, \code{\link{parametric.nlsfit}()},
#' \code{\link{plot.bootstrapfit}()}, \code{\link{print.bootstrapfit}()},
#' \code{\link{simple.nlsfit}()}, \code{\link{summary.bootstrapfit}()}
#' @examples
#' 
#' x <- 1:3
#' dx <- 1:3 * 0.1
...
#' @param boot.R numeric. Number of bootstrap samples to generate.
#' @param x numeric vector. Actual values for the data.
#' @param cov numeric matrix, square, length of \code{x} or missing. Covariance
...
#' as many rows as \code{boot.R}.
#' @seealso Other NLS fit functions: \code{\link{bootstrap.nlsfit}()},
#' \code{\link{parametric.bootstrap}()}, \code{\link{parametric.nlsfit.cov}()},
#' \code{\link{parametric.nlsfit}()}, \code{\link{plot.bootstrapfit}()},
#' \code{\link{print.bootstrapfit}()}, \code{\link{simple.nlsfit}()},
...
#' 
#' @export parametric.bootstrap.cov
...
#' 
#' 
#' @param fn \code{fn(par, x, ...)}. The (non-linear) function to be fitted to
#' the data. Its first argument must be the fit parameters named \code{par}.
#' The second must be \code{x}, the explaining variable. Additional parameters
#' might be passed to the function. Currently we pass \code{boot.r} which is
#' \code{0} for the original data and the ID (1, ...) of the bootstrap sample
#' otherwise. As more parameters might be added in the future it is recommended
#' that the fit function accepts \code{...} as the last parameter to be forward
#' compatible.
...
#' the fit function.
#' @param dy Numeric vector. Errors of the dependent and independent variable,
#' respectively. These do not need to be specified as they can be computed from
#' the bootstrap samples. In the case of parametric bootstrap it might would
#' lead to a loss of information if they were computed from the
...
#' numerical vector.
#' @param dx Numeric vector. Errors of the dependent and independent variable,
#' respectively. These do not need to be specified as they can be computed from
#' the bootstrap samples. In the case of parametric bootstrap it might would
#' lead to a loss of information if they were computed from the
#' pseudo-bootstrap samples. They must not be specified if a covariance matrix
#' is given.
#' @param lower Numeric vector of length \code{length(par.guess)} of lower
#' bounds on the fit parameters. If missing, \code{-Inf} will be set for all.
#' @param upper Numeric vector of length \code{length(par.guess)} of upper
#' bounds on the fit parameters. If missing, \code{+Inf} will be set for all.
#' @param ... Additional parameters passed to \code{fn}, \code{gr} and
#' \code{dfn}.
#' @param bootstrap Shall the error calculation be performed using boostrap? If
#' not, the errors are estimated with help of the jacobian (either provided in
#' \code{gr} or calculated using the \code{numDeriv}-package).
#' @param na.rm logical. If set to \code{true}, NAs in \code{y} and \code{dy}
#' will be ignored. If x-errors are taken into account, NAs in \code{x} and
#' \code{dx} will be ignored, too.
#' @seealso Other NLS fit functions: \code{\link{bootstrap.nlsfit}()},
#' \code{\link{parametric.bootstrap.cov}()},
#' \code{\link{parametric.bootstrap}()}, \code{\link{parametric.nlsfit.cov}()},
#' \code{\link{plot.bootstrapfit}()}, \code{\link{print.bootstrapfit}()},
...
#' @param ... Additional parameters passed to \code{fn}, \code{gr} and
#' \code{dfn}.
#' @param bootstrap boolean. If \code{TRUE}, bootstrap is used.
#' @param na.rm logical. If set to \code{true}, NAs in \code{y} and \code{dy}
#' will be ignored. If x-errors are taken into account, NAs in \code{x} and
#' \code{dx} will be ignored, too.
#' @seealso Other NLS fit functions: \code{\link{bootstrap.nlsfit}()},
#' \code{\link{parametric.bootstrap.cov}()},
#' \code{\link{parametric.bootstrap}()}, \code{\link{parametric.nlsfit}()},
#' \code{\link{plot.bootstrapfit}()}, \code{\link{print.bootstrapfit}()},
#' \code{\link{simple.nlsfit}()}, \code{\link{summary.bootstrapfit}()}
#' @export parametric.nlsfit.cov
...
#' @param x values of the explaining variable in form of a one-dimensional
#' numerical vector.
#' @param errormodel Either "yerror" or "xyerror", depending on the x-values
#' having errors or not.
#' @param priors List possessing the elements \code{param}, \code{p} and
#' \code{psamples}. The vector \code{param} includes the indices of all fit
#' parameters that are to be constrained and the vector \code{p} the
#' corresponding paramater values (e.g. known from a previous fit). The list
#' element \code{psamples} is a matrix of dimensions \code{(boot.R,
#' length(param))} and contains the corresponding bootstrap samples. If this
#' list is not specified priors are omitted within the fit.
#' @param ... Additional parameters passed to \code{fn}, \code{gr} and
#' \code{dfn}.
#' @param lower Numeric vector of length \code{length(par.guess)} of lower
#' bounds on the fit parameters. If missing, \code{-Inf} will be set for all.
...
#' length(y)+length(x))} depending on the errormodel. Pass \code{NULL} if the
#' matrix has to be calculated from the \code{bsamples}. In that case, if the
#' number of boostrap samples is small compared to the number of variables,
#' singular value decomposition with small eigenvalue replacement will be used
#' (see \link{invertCovMatrix}) to attempt a clean inversion. In case a
#' variance-covariance matrix is passed, the inversion will simply be attempted
#' using \code{solve} on the Cholesky decomposition. Finally, if
#' \code{CovMatrix} is missing, an uncorrelated fit will be performed.
#' @param boot.R If larger than 0, \code{boot.R} paramtetric bootstrap samples
#' are generated on the fit results after fit and error calculation are
#' finished. The original data is never boostraped in this function.
#' @param gr \code{gr(par, x, ...)}. \code{gr=d(fn) / d(par)} is a function to
#' return the gradient of \code{fn}. It must return an array with
#' \code{length(x)} rows and \code{length(par)} columns.
#' @param dfn \code{dfn(par, x, ...)}. \code{dfn=d(fn) / dx} is the canonical
...
#' @param success.infos integer vector. When using \code{minpack.lm} there is
#' the \code{info} in the return value. Values of 1, 2 or 3 are certain
#' success. A value of 4 could either be a success or a saddle point. If you
#' want to interpret this as a success as well just pass \code{1:4} instead of
#' the default \code{1:3}.
#' @param relative.weights are the errors on y (and x) to be interpreted as
#' relative weights instead of absolute ones? If TRUE, the covariance martix of
#' the fit parameter results is multiplied by chi^2/dof. This is the default in
#' many fit programs, e.g. gnuplot.
#' @param na.rm logical. If set to \code{true}, NAs in \code{y} and \code{dy}
#' will be ignored. If x-errors are taken into account, NAs in \code{x} and
#' \code{dx} will be ignored, too.
#' @seealso Other NLS fit functions: \code{\link{bootstrap.nlsfit}()},
...
#' \code{\link{parametric.bootstrap}()}, \code{\link{parametric.nlsfit.cov}()},
#' \code{\link{parametric.nlsfit}()}, \code{\link{plot.bootstrapfit}()},
#' \code{\link{print.bootstrapfit}()}, \code{\link{summary.bootstrapfit}()}
#' @examples
#' 
#' ## Declare some data.
#' value <- c(0.1, 0.2, 0.3)
...
#' derivative of \code{fn} by \code{x} and only relevant if x-errors are
#' provided.
#' @param mask logical or integer index vector. The mask is applied to select
#' the observations from the data that are to be used in the fit. It is applied
#' to \code{x}, \code{y}, \code{dx}, \code{dy}, \code{bsamples} and
#' \code{CovMatrix} as applicable.
#' @param use.minpack.lm use the \code{minpack.lm} library if available. This
#' is usually faster than the default \code{optim} but somtimes also less
#' stable.
...
#' estimate. This is a parameter in order to support different resampling
#' methods like jackknife.
#' @param cov_fn function. Function to compute the covariance (matrix). Default
#' is \link{cov}.
#' @param maxiter integer. Maximum number of iterations that can be used in the
#' optimization process.
#' @param success.infos integer vector. When using \code{minpack.lm} there is
...
#' will be ignored. If x-errors are taken into account, NAs in \code{x} and
#' \code{dx} will be ignored, too.
#' @return returns a list of class 'bootstrapfit'. It returns all input
#' parameters and adds in addition the following: \item{t0}{the one dimensional
#' numerical vector of length \code{npar+1}. \code{npar} is the number of fit
#' parameters. In case of 'yerrors' this equals \code{length(par.guess)}. For
#' 'xyerrors' this equals \code{length(par.guess) + length(x)}. \code{t0}
#' contains the best fit parameters obtained on the original data. The last
#' element in \code{t0} is the chisquare value.} \item{t}{an array of
#' dimensions \code{(npar+1, boot.R)} with \code{npar} as in \code{t0}. The
#' rows contain the individual bootstrap observations.} \item{bsamples}{the
#' bootstrap samples used as an array of dimensions \code{(length(y), boot.R)}
#' or \code{(length(y)+length(x), boot.R)} depending on the error model with
#' \code{npar} as in \code{t0}. } \item{Qval}{the p-value of the fit on the
#' original data} \item{chisqr}{the residual chisqr value.} \item{dof}{the
#' residual degrees of freedom of the fit.} \item{nx}{the number of x-values.}
#' \item{tofn}{ the original \code{...} list of parameters to be passed on to
#' the fit function} \item{mask}{original \code{mask} value}
#' @seealso Other NLS fit functions: \code{\link{parametric.bootstrap.cov}()},
#' \code{\link{parametric.bootstrap}()}, \code{\link{parametric.nlsfit.cov}()},
#' \code{\link{parametric.nlsfit}()}, \code{\link{plot.bootstrapfit}()},
#' \code{\link{print.bootstrapfit}()}, \code{\link{simple.nlsfit}()},
#' \code{\link{summary.bootstrapfit}()}
...
#' @param ... Additional parameters passed to the \code{summary.bootstrapfit}
#' function.
#' @param digits number of significant digits to print in summary or print.
#' @seealso Other NLS fit functions: \code{\link{bootstrap.nlsfit}()},
#' \code{\link{parametric.bootstrap.cov}()},
...
#' 
#' @param x object returned by \code{bootstrap.nlsfit}
#' @param ... Additional parameters passed to the \code{plotwitherror}
#' function.
...
#' \code{\link{parametric.bootstrap.cov}()},
#' \code{\link{parametric.bootstrap}()}, \code{\link{parametric.nlsfit.cov}()},
#' \code{\link{parametric.nlsfit}()}, \code{\link{print.bootstrapfit}()},
#' \code{\link{simple.nlsfit}()}, \code{\link{summary.bootstrapfit}()}
