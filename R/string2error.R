#' string2error
#' 
#' takes a string of the form "x(dx)", where dx are the error digits and
#' returns a numeric vector c(x, y), where y is dx as a proper numeric value.
#' 
#' can be used in combination with \link{apply}
#' 
#' @param x Input character string.
#' @return a numeric vector with the first element the value and the second the
#' error
#' @examples
#' 
