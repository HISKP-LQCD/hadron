kaon <- function(cmicor, mu=0.1, kappa=0.156, t1, t2, S=1.5, pl=FALSE, skip=0,
                variational=list(ta=3, tb=4, N=6), ind.vec=c(2,4,5,6,7,8),
  ...
  # change to complex values
  tmp <- complex(real=cmicor[,ind.vec[3]], imag=cmicor[,ind.vec[4]])
  Z <- array(tmp[((Skip):Length)], 
             dim=c(nrObs*(T1)*4,(length(tmp[((Skip):Length)])/(nrObs*(T1)*4))))

  tmp <- complex(real=cmicor[,ind.vec[5]], imag=cmicor[,ind.vec[6]])
  # negative times
  W <- array(tmp[((Skip):Length)], 
             dim=c(nrObs*(T1)*4,(length(tmp[((Skip):Length)])/(nrObs*(T1)*4))))
...
arrangeCor.kaon <- function(T1, W, Z) {
