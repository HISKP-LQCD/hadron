...
#' 
#' 
#' @param par Numeric vector: Fit parameters of the model. In an object of type
#' \code{matrixfit}, this should be located at \code{$opt.res$par}.
#' @param t Numeric vector: Time of interest.
#' @param T Numeric: Time extent of the lattice.
#' @param parind See \code{\link{matrixfit}}.
#' @param sign.vec Numeric vector: Relative sign between forward and backwards
#' propagating part. A plus makes it cosh, a minus makes it sinh.
...
#' @param reference_time Numeric: GEVP reference time value in physical time
#' convention
#' @seealso \code{\link{matrixfit}}
...
#' @param plot.raw Boolean: plot the raw data or multiply out the leading
#' exponetial behaviour
#' @param rep Boolean: whether or not to add to existing plot
#' @param col String vector: vector of colours for the different correlation
#' functions
#' @param every Fit only a part of the data points. Indices that are not
#' multiples of \code{every} are skipped. If no value is provided, all points
#' are taken into account.
#' @param ... Graphical parameters to be passed on to \link{plot} or
#' \link{plotwitherror}.
#' @seealso \code{\link{matrixfit}}
...
#' want.
#' 
#' The correlation function is altered on the time slices which are earlier
#' than the start of the fit interval. The correlator is replaced by the model
...
#' 
#' @param cf Correlation function of class \code{cf}.
#' @param mfit Fit result of class \code{matrixfit}.
...
#' @return A correlation function of class \code{cf} which is computed from the
#' old correlation function \eqn{C(t)} as \eqn{M(t) + C(t) - \bar{C}(t)}, where
