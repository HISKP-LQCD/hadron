...
tseries_apply_reduction <- function(data, reduction, explanatory_vars = NULL) {
  ...
  return(
    dplyr::summarise(
      data_ref,
      dplyr::across(.cols = setdiff(colnames(data_ref), not_across_vars),
                    .fns = reduction),
      .groups = "drop"
    )
  )
...
bootstrap.tseries <- function(.tseries, 
  ...
  if( !parallel_avail & serial == FALSE ){
    stop(sprintf("The 'parallel' package in version is required to use this function with `serial = FALSE`!\n"))
  }
    ...
    FUN = function(row_idx)
      {
        ...
        # of course, this would be much cleaner-looking with pipes, but one should not
        # use them in packages...

        # we resample the data frame, this involves copies of potentially large
        # data structures but I don't see a way around for now
        df_resample <- dplyr::slice(.data = .tseries$data, df_sample_idcs)
        return(
          dplyr::mutate(
            tseries_apply_reduction(data = df_resample,
                                    reduction = mean,
                                    explanatory_vars = .tseries$explanatory_vars),
            sample_idx = row_idx
          )
...
apply_unary_elementwise.tseries <- function(ts, fn){
  ...
  obs_vars <- setdiff(colnames(ts$samples), c(ts$explanatory_vars, "sample_idx"))
...
apply_binary_elementwise.tseries <- function(x, y, `%op%`) {
  ...
  obs_vars <- setdiff(colnames(samples), c(x$explanatory_vars, "sample_idx"))
  ...
  return(
    tseries_boot(boot.R = x$boot.R,
                 ...
                 sample_idcs = ifelse(all(res_compat$sample_idcs), x$sample_idcs, NULL),
