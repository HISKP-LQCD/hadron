...
gf_make_plots <- function(gf_analysis_results, outputbasename, plotsize){
  ...
  for( iter in loop_plan ){
    ...
    gf_plot_flow(obs_uwerr = gf_analysis_results$uw[[iter$name]]$obs,
                 ...
                 intersection = list(obsval = 0.3, tval = scale_sq, 
...
bootstrap_gf_scales_and_ratios <- function(raw_gradflow, boot.l, boot.R, seed = 12345){
  dplyr_avail <- requireNamespace("dplyr")
  if( !dplyr_avail ){
    stop("The 'dplyr' package is required to use this function!\n")
  }
  ...
  plan <- list()
  plan[["sqrt_t0plaq"]] <- expression( sqrt( approx(x = tsqEplaq, y = t, xout = 0.3)$y ) )
  plan[["sqrt_t0sym"]] <- expression( sqrt( approx(x = tsqEsym, y = t, xout = 0.3)$y ) )
