...
#'
#' @param pgevm an object of class 'PGEVM' generated by 'bootstrap.pgevm' or 'bootstrap.truncated.pgevm'
...
#' @family hankel
#' @seealso input is generated via \link{bootstrap.pgevm} or \link{bootstrap.truncated.pgevm}.
...
#'
#' @return
#' Returns an object of S3 class `effectivemass` with the coefficients as well as the energies.
#' 
#' @export
pgevm2coefficients <- function (pgevm, id = c(1), type = "log", eps = 1e-16, n.max, 
                          probs = c(0.16, 0.84), errortype = "outlier-removal", bias_correction = FALSE, 
                          average.negE = FALSE, range = c(0.1, 1)) {
  ...
  if (missing(n.max)) 
    n.max <- max(pgevm$n)
  n.max <- min(n.max, max(pgevm$n))
  if (is.null(pgevm$ndep.Delta)) 
    pgevm$ndep.Delta <- FALSE
  if (is.null(pgevm$block.Delta)) 
    pgevm$block.Delta <- FALSE
  deltat <- pgevm$deltat
  dbboot <- inherits(pgevm$cf, "cf_dbboot")
  if (errortype == "dbboot") {
    if (!dbboot) 
  ...
  effMass.tsboot <- array(NA,dim=c(pgevm$boot.R,max(pgevm$n)))
  coeffs <- c()
  coeffs.tsboot <- array(NA, dim = c(pgevm$boot.R, max(pgevm$n)))
  indexClosest.tsboot <- array(NA, dim = c(pgevm$boot.R,max(pgevm$n)))
  
  coeffs.dbboot <- array()
  effMass.dbboot <- array()
  if (dbboot) {
    dbboot.R <- pgevm$cf$doubleboot$dbboot.R
    effMass.dbboot <- array(NA, dim = c(pgevm$boot.R,dbboot.R,max(pgevm$n)))
    coeffs.dbboot <- array(NA, dim = c(pgevm$boot.R,dbboot.R,max(pgevm$n)))
    indexClosest.dbboot<- array(NA, dim = c(pgevm$boot.R,dbboot.R,max(pgevm$n)))
  }
  ...
  if (average.negE) {
    neffMass.tsboot <- array(NA, dim=c(pgevm$boot.R, max(pgevm$n)))
	nCoeffs.tsboot <- array(NA,dim=c(pgevm$boot.R,max(pgevm$n)))
	nIndexClosest.tsboot <- array(NA, dim = c(pgevm$boot.R,max(pgevm$n)))

    neffMass.dbboot <- array()
	nCoeffs.dbboot <- array()
    if(dbboot) {
      neffMass.dbboot <- array(NA, dim=c(pgevm$boot.R, pgevm$cf$doubleboot$dbboot.R, max(pgevm$n)))
      nCoeffs.dbboot <- array(NA, dim=c(pgevm$boot.R, pgevm$cf$doubleboot$dbboot.R, max(pgevm$n)))
	  nIndexClosest.dbboot <- array(NA, dim = c(pgevm$boot.R,dbboot.R,max(pgevm$n)))

    }
  }
  
  .fn <- function(evs, range, eps, n = length(evs), revert = FALSE) {
    ii <- which(abs(Im(evs)) <= eps & Re(evs) > range[1] & Re(evs) < range[2]) 
    x <- Re(evs[ii])
    ...
    N <- length(x)
    ret = list(value=c(x,rep(NA,times= n - N)),index=c(ii,rep(NA,times=n-N)))
    return(ret)
  }
  
  .closest <- function(x, ref) {
    ii <- which.min(abs(x - ref))
    y <- x[ii]
    if (length(y) == 0){
      ret = list(value=NA,index=NA)
      return(ret)
    }
    ret = list(value=y,index=ii)
    return(ret)
  }
  .closest2 <- function(x, ref) {
    ii <-which.min(abs(x - ref))
    y <- x[ii]
    #print(ii)
    if (length(y) == 0) 
  ...
  for (n in c(1:n.max)) {
    ...
    tmpboot <- apply(X=pgevm$evs.tsboot[,n,ii,drop=FALSE],MARGIN = 1, FUN = .fn, range=range, eps=eps)
    tmpboot.value <- sapply(tmpboot, function(x) x$value ,simplify = "matrix")
    tmpboot.index <-sapply(tmpboot, function(x) x$index ,simplify = "matrix")

    if (dbboot) {
      tmpdbboot <- apply(X = pgevm$evs.dbboot[, , n, ii, drop = FALSE], MARGIN = c(1L, 2L), FUN = .fn, range = range, eps = eps)
      tmpdbboot.value <- sapply(tmpdbboot, function(x) x$value )
      tmpdbboot.index <- sapply(tmpdbboot, function(x) x$index  )
      }
    
    #TODO: Modify 1 for corrMatrix. 
    
    if (n.end == 1) {
      coeffs[n] <- Re(pgevm$coefficients[1,1,1]) #Here get the correct one.
      coeffs.tsboot[,n] <- Re(pgevm$coefficients.tsboot[,1,1,1])
      
      effMass[n]<- tmp$value
      effMass.tsboot[,n] <- tmpboot.value

      if (dbboot) {
        effMass.dbboot[ , ,n] <- tmpdbboot.value
        coeffs.dbboot[,,n]<-Re(pgevm$coefficients.dbboot[,,1,1,1])
      }
    }
    else {
      med <- median(c(tmp$value[id],tmpboot.value[id,]), na.rm = TRUE)
      effMass[n] <- .closest(tmp$value, ref = med)$value
      
      closest.tsboot <- apply(tmpboot.value, MARGIN=2L, FUN=.closest, med)
      effMass.tsboot[,n] <- sapply(closest.tsboot, function(x) x$value)
      
      tmpCoeffs <- Re(pgevm$coefficients[n,tmp$index,1])
      tmpCoeffs.tsboot <- sapply(1:pgevm$boot.R, function(bs){Re(pgevm$coefficients.tsboot[bs,n,tmpboot.index[,bs],1])})
      
      #Closest for value
      indexClosest <- .closest(tmp$value, ref = med)$index
      coeffs[n] <- tmpCoeffs[indexClosest]
      
      #closest for tsboot
      indexClosest.tsboot[,n]<- sapply(closest.tsboot, function(x) x$index ,simplify = "matrix")
      coeffs.tsboot[,n] <- sapply(1:pgevm$boot.R, function(bs){tmpCoeffs.tsboot[indexClosest.tsboot[bs,n],bs]})
      
      if (dbboot) {
        tmpdbboot <- array(tmpdbboot.value, dim= c(n,pgevm$boot.R,dbboot.R))
        closest.dbboot <- apply(tmpdbboot, MARGIN = c(2L, 3L), FUN = .closest, ref = med)
        
        index.tmpdbboot <- array(tmpdbboot.index, dim= c(n,pgevm$boot.R,dbboot.R))
        idx <- expand.grid(bs = seq_len(pgevm$boot.R),dbs = seq_len(dbboot.R))
        tmpCoeffs.dbboot <- apply(idx, 1, function(z) {
          bs  <- z[1]
          dbs <- z[2]
          Re(pgevm$coefficients.dbboot[bs, dbs, n, index.tmpdbboot[, bs, dbs],1])
        })

        coeffs.tmpdbboot <- array(tmpCoeffs.dbboot,dim=c(n,pgevm$boot.R,dbboot.R))
        closest.dbboot <- apply(tmpdbboot, MARGIN = c(2L, 3L), FUN = .closest, ref = med)
        
        
        for (bs in (1:pgevm$boot.R)) {
          for (dbs in (1:dbboot.R)) {
            effMass.dbboot[bs,dbs ,n] <- closest.dbboot[[bs,dbs]]$value
            
            indexClosest.dbboot[bs,dbs ,n] <- closest.dbboot[[bs,dbs]]$index
            coeffs.dbboot[bs,dbs,n] <- coeffs.tmpdbboot[indexClosest.dbboot[bs,dbs,n],bs,dbs]
          }
        }#END Loops
  ...
  if (dbboot) {
    effMass.tsboot <- apply(effMass.dbboot, MARGIN = c(1L,3L), FUN = median, na.rm = TRUE)
    coeffs.tsboot <- apply(coeffs.dbboot,MARGIN=c(1L,3L), FUN= median, na.rm=TRUE)
  }

  effMass <- -log(effMass)/deltat
  effMass.tsboot <- -log(effMass.tsboot)/deltat
  biasMass <- effMass - apply(effMass.tsboot, MARGIN=2L, FUN= median, na.rm = TRUE)
  
  coeffs <- sqrt(coeffs)*sqrt(2)
  coeffs.tsboot <- sqrt(coeffs.tsboot)*sqrt(2)

  biasCoeffs <- coeffs - apply(coeffs.tsboot, MARGIN=2L, FUN= median, na.rm = TRUE)
  
  if (bias_correction) {
    effMass <- effMass -biasMass
    coeffs <- coeffs -biasCoeffs
  }
  ...
  if (average.negE) {
	...
	for(n in c(2:n.max)){
      ...
      #TODO: Modify 1 for corrMatrix. This ask Johan because I do not Understand
      
	  if (n.end == 1) {
        nCoeffs[n] <- Re(pgevm$coefficients[1,1,1]) #Here get the correct one.
        nCoeffs.tsboot[,n] <- Re(pgevm$coefficients.tsboot[,1,1,1])
        
        neffMass[n]<- tmp$value
        neffMass.tsboot[,n] <- tmpboot.value

        if (dbboot) { #Taken form pgevm2effmass; is it ever true?
          neffMass.dbboot[ , ,n] <- tmpdbboot.value
          nCoeffs.dbboot[,,n]<-Re(pgevm$coefficients.dbboot[,,1,1,1])
        }
	  }
	  else{
        med <- median(c(tmp$value[id],tmpboot.value[id,]), na.rm = TRUE)
        neffMass[n] <- .closest(tmp$value, ref = med)$value
        
        closest.tsboot <- apply(tmpboot.value, MARGIN=2L, FUN=.closest, med)
        neffMass.tsboot[,n] <- sapply(closest.tsboot, function(x) x$value)
        ...
        #Closest for value
        indexClosest <- .closest(tmp$value, ref = med)$index
        nCoeffs[n] <- tmpCoeffs[indexClosest]
        
        #closest for tsboot
        nIndexClosest.tsboot[,n]<- sapply(closest.tsboot, function(x) x$index ,simplify = "matrix")
        nCoeffs.tsboot[,n] <- sapply(1:pgevm$boot.R, function(bs){tmpCoeffs.tsboot[nIndexClosest.tsboot[bs,n],bs]})
        
        if (dbboot) {
          tmpdbboot <- array(tmpdbboot.value, dim= c(n,pgevm$boot.R,dbboot.R))
          closest.dbboot <- apply(tmpdbboot, MARGIN = c(2L, 3L), FUN = .closest, ref = med)
          
          nIndex.tmpdbboot <- array(tmpdbboot.index, dim= c(n,pgevm$boot.R,dbboot.R))
          idx <- expand.grid(bs = seq_len(pgevm$boot.R),dbs = seq_len(dbboot.R))
          tmpCoeffs.dbboot <- apply(idx, 1, function(z) {
            bs  <- z[1]
            dbs <- z[2]
          ...
          for (bs in (1:pgevm$boot.R)) {
            for (dbs in (1:dbboot.R)) {
              neffMass.dbboot[bs,dbs ,n] <- closest.dbboot[[bs,dbs]]$value
              
              nIndexClosest.dbboot[bs,dbs ,n] <- closest.dbboot[[bs,dbs]]$index
              nCoeffs.dbboot[bs,dbs,n] <- coeffs.tmpdbboot[nIndexClosest.dbboot[bs,dbs,n],bs,dbs]
            }
          }#END Loops
        }#end if dboot     
      }#end else
    ...
    if (dbboot) {
      neffMass.tsboot <- apply(neffMass.dbboot, MARGIN = c(1L,3L), FUN = median, na.rm = TRUE)
      nCoeffs.tsboot <- apply(nCoeffs.dbboot,MARGIN=c(1L,3L), FUN= median, na.rm=TRUE)
    }

    neffMass <- -log(neffMass)/deltat
    neffMass.tsboot <- -log(neffMass.tsboot)/deltat
    nbiasMass <- neffMass - apply(neffMass.tsboot, MARGIN=2L, FUN= median, na.rm = TRUE)
    
    nCoeffs <- sqrt(nCcoeffs)*sqrt(2)
    nCoeffs.tsboot <- sqrt(nCoeffs.tsboot)*sqrt(2)

    biasCoeffs <- nCoeffs - apply(nCoeffs.tsboot, MARGIN=2L, FUN= median, na.rm = TRUE)
    
    if (bias_correction) {
      neffMass <- neffMass -nbiasMass
      nCoeffs <- nCoeffs -nbiasCoeffs
    }
  }#end ifNegEnergy 
  ################################################################
  deffMass <- c()
  ...
  if (errortype=="outlier-removal") {
	  ...
	  effMass.tsboot <- apply(effMass.tsboot,2,remove_outliers)
	  coeffs.tsboot	<- apply(coeffs.tsboot,2,remove_outliers)

	  deffMass <- apply(effMass.tsboot,MARGIN=2L,FUN=pgevm$cf$error_fn,na.rm=TRUE)
	  dCoeffs <- apply(coeffs.tsboot,MARGIN=2L,FUN=pgevm$cf$error_fn,na.rm=TRUE)
  }
  else if (errortype=="quantiles") {
	  error_fn <- function(x,probs=c(0.16,0.84)){
		  Q <- quantile(x,probs=probs,na.rm=TRUE)
	  ...
	  deffMass <-apply(effMass.tsboot,MARGIN=2L,FUN=error_fn,probs=probs)
	  dCoeffs <-apply(coeffs.tsboot,MARGIN=2L,FUN=error_fn,probs=probs)
  }
  else{
	  if(average.negE){
		deffMass <- apply((effMass.tsboot+neffMass.tsboot)/2, MARGIN=2L, FUN=sd, na.rm=TRUE)
		dCoeffs <- apply((coeffs.tsboot+nCoeffs.tsboot)/2, MARGIN=2L, FUN=sd, na.rm=TRUE)
	  }
	  else{
		deffMass <- apply(effMass.tsboot, MARGIN=2L, FUN=sd, na.rm=TRUE)
		dCoeffs <- apply(coeffs.tsboot, MARGIN = 2L, FUN=sd, na.rm=TRUE)
	  }
  }

  if (average.negE){
	  effMass <- (effMass+neffMass)/2
	  coeffs <- (coeffs+nCoeffs)/2
  }
  if (pgevm$block.Delta) {
    t.idx = pgevm$n/2
    effMass = effMass[pgevm$n]
	...
	coeffs = coeffs[pgecm$n]
	dCoeffs = dCoeffs[pgevm$n]
	nCoeffs = nCoeffs[pgevm$n]
	coeffs.tsboot	= coeffs.tsboot[,pgevm$n]
  }
  else{
	t.idx = c(1:n.max)
  }
  ...
  ret <- list(t.idx = t.idx, 
			  ...
			  cf = pgevm$cf,
              coeffs=coeffs, 
			  coeffs.tsboot = coeffs.tsboot, 
			  coeffs.dbboot = coeffs.dbboot, 
			  dCoeffs=dCoeffs,
              effMass = effMass, 
			  effMass.tsboot = effMass.tsboot,
              ...
              n.max = n.max, 
			  type = "log", 
              opt.res = NULL, t1 = NULL, t2 = NULL, useCov = NULL, 
              CovMatrix = NULL, invCovMatrix = NULL,
			  boot.R = pgevm$boot.R,boot.l = pgevm$boot.l,
			  seed = pgevm$seed, 
			  biasCoeffs = biasCoeffs, 
			  nbiasCoeffs=nbiasCoeffs,
              biasMeff = biasMass,
			  nbiasMass	=nbiasMass,
			  massfit.tsboot = NULL, Time = pgevm$cf$Time, 
              N = 1, nrObs = 1, dof = NULL, chisqr = NULL, Qval = NULL, 
              errortype = NULL)
  
  attr(ret, "class") <- c("coefficients", "PGEVM", class(ret))
  return(invisible(ret))
}
...
pgevm2fps <- function (pgevm, id = c(1), type = "log", eps = 1e-16, n.max, 
                            probs = c(0.16, 0.84), errortype = "outlier-removal", bias_correction = FALSE, 
                            average.negE = FALSE, range = c(0.1, 1),
                            mu1,mu2,Kappa,normalisation="cmi",disprel="continuum") {
  ...
  pgevm.coeffs <- pgevm2coefficients(pgevm, id = id, type = type, eps = eps, n.max, 
                                     probs = probs, errortype = errortype, bias_correction = bias_correction, 
                                     average.negE = average.negE, range = range)
  
  ######HERE WE CALCULATE THE DECAY CONSTANTS ####################
  
  fps <- c()
  fps.tsboot <- c()
  
  k<- Kappa
  if(normalisation!="cmi") {
	  k <- sqrt(0.5)
  }
  
  denominator <- sqrt(pgevm.coeffs$effMass^3)
  if(disprel == "lattice"){
	  denominator <- sinh(pgevm.coeffs$effMass)*sqrt(pgevm.coeffs$effMass) 
  }
  fps <- 2*k*(mu1+mu2)/sqrt(2) * abs(pgevm.coeffs$coeffs)/denominator
  denominator.tsboot <- sqrt(pgevm.coeffs$effMass.tsboot^3)
  if(disprel=="lattice") { denominator.tsboot <- sinh(pgevm.coeffs$effMass.tsboot)*sqrt(pgevm.coeffs$effMass.tsboot) }
  fps.tsboot <- 2*k*(mu1+mu2)/sqrt(2) * abs(pgevm.coeffs$coeffs.tsboot)/denominator.tsboot
  dfps <-  apply(fps.tsboot,MARGIN=2L,FUN=sd,na.rm=TRUE)
  
  ret <- list(t.idx = pgevm.coeffs$t.idx, 
              pgevm = pgevm, 
              cf = pgevm$cf,
              fps=fps, 
              fps.tsboot = fps.tsboot, 
              dfps=dfps,
              effMass = pgevm.coeffs$effMass, 
              effMass.tsboot =pgevm.coeffs$effMass.tsboot,
              effMass.dbboot=pgevm.coeffs$effMass.dbboot,
              deffMass=pgevm.coeffs$deffMass,
              t=fps.tsboot,
              t0=fps,
              se= dfps,
              n.max = pgevm.coeffs$n.max, 
              type = "log", 
              opt.res = NULL, t1 = NULL, t2 = NULL, useCov = NULL, 
              CovMatrix = NULL, invCovMatrix = NULL,
              boot.R = pgevm$boot.R,boot.l = pgevm$boot.l,
...
#'
#' @param x Object of class `coefficients` a.k.a. coefficients extracted from pgevm object. 
...
#' 
#' @export
plot_coefficients <- function(pgevmCoeffs,..., ref.value, col, col.fitline, xshift=0){
	effMassWrapper <- list(t.idx = pgevmCoeffs$t.idx, 
			  pgevm = pgevmCoeffs$pgevm, 
			  cf = pgevmCoeffs$cf,
              effMass = pgevmCoeffs$t0, 
			  effMass.tsboot = pgevmCoeffs$t,
			  deffMass=pgevmCoeffs$se,
			  t0=pgevmCoeffs$t0,
              t=pgevmCoeffs$t,
			  se= pgevmCoeffs$se,
              n.max = pgevmCoeffs$n.max, 
			  type = "log", 
              opt.res = NULL, t1 = NULL, t2 = NULL, useCov = NULL, 
              CovMatrix = NULL, invCovMatrix = NULL,
			  boot.R = pgevmCoeffs$boot.R,boot.l = pgevmCoeffs$boot.l,
			  seed = pgevmCoeffs$seed, 
			  massfit.tsboot = NULL, Time = pgevmCoeffs$cf$Time, 
              N = 1, nrObs = 1, dof = NULL, chisqr = NULL, Qval = NULL, 
              errortype = NULL)

	attr(effMassWrapper,"class") <- c("effectivemass","PGEVM", class(effMassWrapper))
	plot(effMassWrapper,...,ref.value = ref.value, col=col,col.fitline=col.fitline,xshift=xshift)
