...
fit.Na.withr0ZP <- function(data, startvalues, bootsamples, fsmethod="gl", a.guess,
  ...
  r0mu.phys <- uniroot(fovermps.Na.withr0, c(0.0005, 0.020),
  ...
  if(!missing(bootsamples)) {
    ...
    for(s in 1:boot.R) {
      ...
      if(mini.boot$convergence != 0) {
        ...
        r0F <- numeric()
        mpssqr <- numeric()
        boots[s,1] <- uniroot(fovermps.Na.withr0, c(0.0005, 0.020),
        ...
        boots[s,i] <- boots[s,1]/fpi*0.1307
        for(i in 1:N) {
  ...
  result <- list(par=par, result=list(mu.phys=mu.phys, F=F, a=a, l1=l1, l2=l2, l3=l3, l4=l4, r0=r0,
                            B0=B0, mN0=mN0, mN=mN, c1=c1, gA=gA, ZP=Zp, Sigma=Sigma, rssq=rssq,
